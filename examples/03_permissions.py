...
async def delete_post(
    post_id: int,
    ctx: RequestContext = Depends(flow_dependency(delete_flow))
):
...
async def list_posts(
    ctx: RequestContext = Depends(flow_dependency(read_flow))
):
...
if __name__ == "__main__":
    ...
    #   curl -X POST -H "Authorization: Bearer readonly-token" http://localhost:8000/posts  # 403
    #
    # Delete permission:
    #   curl -X DELETE -H "Authorization: Bearer admin-token" http://localhost:8000/posts/1
    #   curl -X DELETE -H "Authorization: Bearer user-token" http://localhost:8000/posts/1  # 403
    #
    # Read permission:
